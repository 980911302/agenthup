/** 协议层副本：desktop/src/composables/useChatRun.js。Run 事件协议变更时两边一起改。 */
import { onBeforeUnmount, ref } from 'vue'
import { confirmDanger, rejectAllConfirms } from '../utils/confirm'
import { declarePayloadFor, markDeclared } from '../chat/clientTools'
import { cancelChatRun, confirmChatTool, createChatRun, getActiveChatRun, getChatRun, getChatRunState } from '../api/chat'
import { chatRpc } from '../api/chatRpc'
import { newTurn, buildTurns, collectKbCitationState } from './useTurnBuilder'
import { EVENT_TYPES, STEP_TYPES, UI_ARTIFACT_NAMES, isSupportedUiArtifact } from '../chat/types'
import { handleToolCallRequest } from '../chat/clientTools'
...
export function useChatRun(options = {}) {
  ...
  async function send(message, payload) {
    ...
    const firstTurnDeclare = declarePayloadFor(payload?.sessionId)
    const request = {
      ...payload,
      ...(firstTurnDeclare || {}),
    ...
    try {
      ...
      if (firstTurnDeclare) markDeclared(payload?.sessionId)
  ...
  function replaceRunTurn(turn, run) {
    ...
    if (index >= 0) {
      const existing = turns.value[index]
      if (existing) {
      ...
      turns.value.splice(index, 1, turn)
    } else {
  ...
  async function abort() {
    // 停止时把未决确认按拒绝了结:这一轮已经不要了,弹窗留在那儿只会误导用户,
    // 对应的工具也拿不到结果、只能干等服务端超时。
    rejectAllConfirms()
  ...
  function promptToolConfirm(event) {
    ...
    confirmDanger(
      '需要确认',
      `智能体请求执行危险操作「${name}」。\n\n${argsPreview || '(无入参预览)'}\n\n是否允许执行？`
    ).then((ok) => confirmChatTool(runId, confirmId, !!ok).catch(() => {}))
  }
  ...
  function applyEvent(turn, event) {
    ...
    switch (event.type) {
      ...
      case EVENT_TYPES.TOOL_CALL_REQUEST: {
        const runId = turn.runId || (activeRun.value && activeRun.value.runId)
        handleToolCallRequest(runId, event)
        break
      }
...
function turnFromRunState(state, run) {
  ...
  for (const ui of uiSteps) {
    ...
    if (ui.name === UI_ARTIFACT_NAMES.WORKSPACE_CHANGES) {
      const files = Array.isArray(ui.payload?.files) ? ui.payload.files : []
      turn.workspaceChanges = mergeWorkspaceChanges(turn.workspaceChanges, files)
      turn.workspaceChangesTruncated = !!(turn.workspaceChangesTruncated || ui.payload?.truncated)
    }
  }
...
function snapshotStep(row, sessionId) {
  const common = {
    stepId: row.stepId,
