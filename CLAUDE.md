...
**改代码前必读**:`docs/` 目录是本项目的权威架构文档(全部中文),含横向总览、阅读地图、会话身份、14 篇模块文档、6 篇模块 API、1 篇表结构,且随代码持续校对同步。实施计划的完工对照见 `docs/实施计划落地状态.md`(文内「待实施」已过期,不要按计划再开工)。请先读 `docs/整体架构总览.md`,再按需读对应 `xxx模块.md` 或 `xxxAPI文档.md`。本文档只做索引与速查,不重复细节。
