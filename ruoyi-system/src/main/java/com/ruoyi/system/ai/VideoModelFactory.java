...
/**
 * 视频模型工厂,与 {@link ImageModelFactory} 平行:按供应选渠道,构造 OpenAI 兼容视频客户端。
 * 不碰生图缓存与 {@code OpenAiImageModel}。
 */
@Component
public class VideoModelFactory
{
    private static final Logger log = LoggerFactory.getLogger(VideoModelFactory.class);
    ...
    private final ConcurrentMap<String, OpenAiCompatibleVideoClient> cache = new ConcurrentHashMap<>();

    public OpenAiCompatibleVideoClient get(Long modelId)
    {
        if (modelId == null)
        {
            throw new ServiceException("请选择视频模型");
        }
        List<AiModelChannel> supplies = aiModelSupplyService.listByModelId(modelId);
        if (supplies == null || supplies.isEmpty())
        {
            throw new ServiceException("该视频模型暂无可用渠道,请先在模型管理中配置供应");
        }
        ...
        return cache.computeIfAbsent(cacheKey, ignored -> {
            log.info("构造 VideoClient:渠道[{}],模型[{}]", channel.getChannelName(), modelName);
            return new OpenAiCompatibleVideoClient(channel.getBaseUrl(), apiKey, modelName);
        });
    }
