import { createChatWebSocketTicket } from './chat'
...
class ChatRpcClient {
  ...
  scheduleReconnect() {
    if (this.closedByClient || this.reconnectTimer || this.connectPromise || this.isOpen()) return
    if (!this.shouldStayConnected()) return
    const delay = Math.min(1000 * (2 ** this.reconnectAttempts), 15000)
    this.reconnectAttempts += 1
    // 握手失败时 connect() 只抛异常不改状态，这里补一次广播，避免指示灯永远停在“连接中”。
  ...
  deliverRunEvent(entry, params) {
    ...
    entry.onEvent && entry.onEvent(params.event || {}, params)
    return true
  }
