  ...
  <div class="chat-input-wrapper">
    ...
    <div v-if="showSlashMenu" class="slash-menu" ref="slashMenuRef">
      <div class="slash-menu__head">
        <span>快捷技能与指令</span>
        <span class="slash-menu__tip">按 ↑↓ 切换，Enter 选用，Esc 关闭</span>
      </div>
    ...
    >
      ...
      <div v-if="selectedSkills.length" class="chat-skill-chips">
      ...
      <textarea
        ...
        rows="1"
        placeholder="随便问点什么——输入 @ 唤出技能，输入 / 使用快捷指令"
        @keydown="onKeydown"
        @input="onInput"
        @focus="focused = true"
        @blur="focused = false"
      ...
      <div class="chat-input__bar">
        ...
        <div class="chat-input__tools">
          ...
          <!-- 2. @ 技能：只展示当前用户自己的技能 -->
          <div class="tool-btn skill-pick" :class="{ 'is-open': skillPickerOpen, 'is-active': selectedSkills.length }" ref="skillPickerRef">
            <button
              type="button"
          ...
          <!-- 5. 知识库挂载入口 (带数量胶囊) -->
          <div class="tool-btn kb-pick" :class="{ 'is-open': kbPickerOpen, 'is-active': kbIds.length > 0 }" ref="kbPickerRef">
            <button
              type="button"
              class="tool-btn__trigger"
            ...
            <div v-if="kbPickerOpen" class="kb-pick__menu">
              ...
              <template v-else-if="kbs.length">
                <p class="kb-pick__tip">勾选后智能体将优先检索指定知识库中的文档资产</p>
        ...
        <div class="chat-input__actions">
          ...
          <div class="tool-btn agent-pick" :class="{ 'is-open': pickerOpen }" ref="pickerRef">
            ...
            >
              ...
              <span class="tool-btn__text">{{ currentModel?.displayName || '默认模型' }}</span>
              <svg class="tool-btn__chev" width="9" height="9" viewBox="0 0 12 12" fill="none"><path d="M2.5 4.5L6 8l3.5-3.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
          ...
          <span v-if="streaming" class="chat-input__status">
            <i class="chat-input__dot chat-input__dot--run"></i>生成中…
          </span>
