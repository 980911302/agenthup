-- ============================================================
-- 知识库域 DDL(PostgreSQL + pgvector)
-- 见 docs/ai/ai-kb-postgres-plan.md
-- ============================================================
...
-- ----------------------------
-- kb_chunk(无 embedding BLOB,向量在 kb_vector_{dim})
-- ----------------------------
...
-- pgvector HNSW 上限 2000 维,3072 只能全表扫(带 kb_id 过滤仍可用)
create index if not exists idx_kb_vec_3072_kb on kb_vector_3072 (kb_id);

-- ----------------------------
-- 第二期:知识图谱状态与缓存(见 sql/kb_graph.sql)
-- ----------------------------
...
-- 图抽取运行记录(证据血统 v2,见 sql/kb_graph_v2.sql)
...
-- Graph TextUnit（与检索 LEAF 解耦，见 sql/kb_graph_text_unit.sql）
