...
**前端页面**:`ruoyi-ui/src/views/ai/{tool,mcpServer,skill}/index.vue` 三个管理页;**API 封装**:`ruoyi-ui/src/api/ai/{tool,mcpServer,skill}.js`。三个子概念的管理面均为**同步 REST**,运行时装配发生在后端 `AgentContextFactory`,前端在 Chat 页通过事件观察工具执行。

| 前端操作 | API 函数 | HTTP | 后端端点 |
| --- | --- | --- | --- |
| 工具列表/启停 | `listTool` / `changeToolStatus` | GET/PUT | `/ai/tool/list` / `/ai/tool/status` |
| MCP 全局同步 | `syncMcpAllTools()` | POST | `/ai/tool/sync/mcp` |
| 单 MCP 同步 | `syncMcpTools(mcpServerId)` | POST | `/ai/tool/sync/mcp/{mcpServerId}` |
| MCP 服务 CRUD | `listMcpServer` / `addMcpServer` / `updateMcpServer` / `delMcpServer` | GET/POST/PUT/DELETE | `/ai/mcpServer...` |
| MCP 重连 | `reconnectMcpServer(mcpServerId)` | POST | `/ai/mcpServer/{serverId}/reconnect` |
| MCP 运行态 | `getMcpRuntimeStatus()` | GET | `/ai/mcpServer/runtime-status` |
| 技能 CRUD | `listSkill` / `getSkill` / `addSkill` / `updateSkill` / `delSkill` | GET/POST/PUT/DELETE | `/ai/skill...` |
| 技能附件 | — | GET/POST/DELETE | `/ai/skill/{skillId}/files...` |

**交互要点**:
- **工具与技能是"声明 → 装配"模式**:管理页只维护工具/技能/服务的元数据(`AiTool` / `AiSkill` / `AiMcpServer`),真正变成模型可调的工具发生在请求期 —— `AgentContextFactory.doBuild()` 把启用的 tool + skill + MCP 工具装配进 `tools` 列表。因此前端改配置后**无需重启**,下一次对话即生效。
- **MCP 同步是重操作**:点「同步」触发 `syncMcpAllTools` / `syncMcpTools`,服务端扫描已注册 MCP 服务器,把远端工具合并进 `ai_tool`(新增/复活),返回 `{added, stats:{providers, builtinCallbacks, mcpTools, total}}`;`runtime-status` 提供每个 server 的连接态,前端轮询或刷新查看。
- **运行时观察在 Chat 页**:工具真正执行时,前端通过 `useChatRun` 消费 `tool_start` / `tool_end` 事件渲染步骤卡片(`steps/ToolStep.vue`);需要人工确认的工具触发 `tool_confirm_required`,弹确认框后把确认结果写回 Run。这些都在 `聊天执行引擎.md` 展开,不在本模块管理页。
- 权限控制:三个页面的增删改按钮均由 `v-hasPermi` 门控(`ai:tool:*` / `ai:mcpServer:*` / `ai:skill:*`)。
