...
**前端页面**:`ruoyi-ui/src/views/ai/model/index.vue`(模型卡片列表 + 详情/编辑/导入 Sheet)+ `supply.vue`(供应渠道管理);**API 封装**:`ruoyi-ui/src/api/ai/model.js`。管理端是纯 REST 管理面,无实时通道。

| 前端操作 | API 函数(`model.js`) | HTTP | 后端端点 | 说明 |
| --- | --- | --- | --- | --- |
| 列表查询 | `listModel(query)` | GET | `/ai/model/list` | 模型卡片,按类型/状态过滤 |
| 详情/编辑回填 | `getModel(modelId)` | GET | `/ai/model/{modelId}` | 编辑 Sheet 回显 |
| 保存修改 | `updateModel(data)` | PUT | `/ai/model` | 触发渠道事件→模型工厂缓存重建 |
| 删除 | `delModel(modelIds)` | DELETE | `/ai/model/{modelIds}` | 批量删除 |
| 读取上游清单 | `listUpstreamModels(channelId)` | GET | `/ai/model/import/upstream` | 导入前选渠道,展示渠道已维护模型 |
| 导入模型 | `importModel(data)` | POST | `/ai/model/import` | 不存在则建模型,已存在只加供应 |
| 供应列表 | `listModelSupply(modelId)` | GET | `/ai/model/{modelId}/supply` | `supply.vue` 进入时加载 |
| 供应候选 | `listSupplyCandidates(modelId)` | GET | `/ai/model/{modelId}/supply/candidates` | 新增供应时可选渠道/模型清单 |
| 新增供应 | `addModelSupply(modelId, data)` | POST | `/ai/model/{modelId}/supply` | modelId 从路径注入 |
| 修改供应 | `updateModelSupply(data)` | PUT | `/ai/model/supply` | 权重/重试/价格/状态 |
| 删除供应 | `delModelSupply(ids)` | DELETE | `/ai/model/supply/{ids}` | 软删 |

**交互要点**:
- **导入向导**是模型模块交互的核心:`index.vue` → 选渠道 → `listUpstreamModels(channelId)` 拉该渠道模型清单(为空则提示"去渠道管理同步")→ `importModel` 提交,服务端在事务内"建模型 + 建供应"。
- 供应(`supply.vue`)与渠道(`channel/index.vue`)联动:渠道新增/修改经 `AiChannelController` 发 `AiChannelChangedEvent`,三个 ModelFactory 监听后**无重启重建缓存**,前端下一次 `listModel` 即看到新供应。
- 前端能力徽标(`recommendType` / `capabilities`)直接来自导入时记录的字段,不做二次计算;这些字段在 `import/upstream` 时由后端按上游模型推荐。
- 无 WebSocket/SSE;所有交互为同步 REST,权限由 `v-hasPermi` 控制。
