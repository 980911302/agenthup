...
**管理面(REST,纯配置)**:`ruoyi-ui/src/views/ai/agent/index.vue` + `AgentTopology.vue` + `CapabilityPicker.vue`,调用 `api/ai/agent.js`,端点见 §4。配置内容为:基本信息、模型绑定、技能/工具/子智能体关联、系统提示词、推理开关等。

**运行面(WebSocket + 事件,消费装配产物)**:

| 环节 | 前端 | 后端 | 说明 |
| --- | --- | --- | --- |
| 发起对话 | Chat 页 → `useChatRun.send()` → `POST /ai/chat/run` | `AiChatRunController` → `ChatRunExecutor` | body 带 `agentId`,服务端按 agent 装配 |
| 装配触发 | 无感知 | `AgentContextFactory.buildForRun()` | 读 `ai_agent` + 关联,拼出 `ChatModel + options + tools + systemPrompt` |
| 子 agent 调用 | Chat 页步骤卡片 | `agent_start` / `agent_end` 事件带 `ownerAgentCode` / `invId` | 前端 `useTurnBuilder` 按 owner 嵌套渲染子 agent 步骤 |
| 工具执行 | `steps/ToolStep.vue` | `tool_start` / `tool_end` 事件 | 事件经 Run 总线 → WebSocket 推前端 |

**交互要点**:
- **配置与运行时解耦**:管理页保存的 `ai_agent` 只是"配方",每次对话在请求期重新装配;改配置后立即影响下一次对话,无需重启。
- **AgentTopology.vue 只读渲染**:`getAgent(agentId)` 详情里的子 agent 关联,前端画拓扑图,**不参与运行时**;真正的递归调用由 `SubAgentToolCallback` 在服务端驱动。
- **子 agent 卡片身份靠事件字段拼装**:前端不主动感知子 agent 存在,而是按 `agent_start/end` 事件的 `ownerAgentCode` + `invId` 把步骤挂到正确的父卡片下 —— 同一子 agent 一轮被调多次时靠 `invId` 区分(曾出现过第 2 次调用步骤错挂第 1 张卡片的 bug,见 §2.3)。
- **CapabilityPicker 是选择器而非运行时**:关联技能/工具/子 agent 时用它多选,提交后存关联表;对话期是否真正启用由服务端状态/预算决定,前端不重复判断。
