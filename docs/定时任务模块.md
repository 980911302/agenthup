...
**前端页面**:`ruoyi-ui/src/views/ai/job/index.vue`(任务管理列表/表单)+ `log.vue`(触发日志);**API 封装**:`ruoyi-ui/src/api/ai/job.js`。

| 前端操作 | API 函数(`job.js`) | HTTP | 后端端点 | 说明 |
| --- | --- | --- | --- | --- |
| 列表查询 | `listJob(query)` | GET | `/ai/job/list` | 搜索表单 + 分页参数 |
| 详情回填 | `getJob(jobId)` | GET | `/ai/job/{jobId}` | 编辑弹窗回显,含 cron 表达式 |
| 新增/编辑保存 | `addJob(data)` / `updateJob(data)` | POST/PUT | `/ai/job` | 保存后服务端重建 Quartz 调度 |
| 启停切换 | `changeJobStatus(jobId, status)` | PUT | `/ai/job/changeStatus` | 行内开关 `el-switch` |
| 立即执行 | `runJob(jobId)` | POST | `/ai/job/run/{jobId}` | 「执行一次」按钮,同步触发调度 |
| cron 预览 | `nextFireTimes(cron, tz)` | GET | `/ai/job/nextFireTimes` | 表单内 `Crontab` 组件实时预览下次触发时刻 |
| 日志查看 | `listJobLog(query)` | GET | `/ai/jobLog/list` | `log.vue` 独立页面 |
| 删除 | `delJob(jobIds)` / `delJobLog(logIds)` | DELETE | `/ai/job/{jobIds}` / `/ai/jobLog/{logIds}` | 批量删除 |

**交互要点**:
- 表单的 cron 表达式用 `@/components/Crontab` 组件编辑,变更时调 `nextFireTimes` 取未来 5 次触发时刻做**前端预览**(默认时区 `Asia/Shanghai`),提交后再由服务端 `validateForWrite` 二次校验。
- 任务状态(`status` 启停)与触发日志(`log.vue`)分属两个菜单页;执行动作(立即执行/启停)是同步请求,结果以 `AjaxResult` 返回,无 WebSocket 推送——任务真正触发后产生的对话走聊天模块的 Run 链路,前端在 Chat 页订阅而非 Job 页。
- 列表接口在非超管账号下按 `create_by` 过滤本人任务,前端菜单权限由 `v-hasPermi` 控制。
