...
**前端消费入口**:`ruoyi-ui/src/views/ai/chat/composables/useChatRun.js` 的 `applyEvent(turn, event)`(`:598-720`),事件经 `chat.run.subscribe` → `chat.event` 到达;事件类型常量在 `useChatRun.js` 顶部 `EVENT_TYPES` / `STEP_TYPES`。

**事件 → 前端渲染映射**:

| 事件 `type` | 前端处理(`useChatRun.js`) | 渲染 |
| --- | --- | --- |
| `text` | `:707` | 追加到 `CONTENT` 步骤的 `answer` 流式文本 |
| `reasoning` | `:604` | `REASONING` 步骤思考条(可折叠) |
| `tool_start` | `:651` | 创建 `TOOL` 步骤卡片,args 脱敏展示 |
| `tool_end` | `:663` | 补 `TOOL` 步骤结果/耗时/附件 |
| `tool_confirm_required` | `:613` | 弹确认框,`confirmId` 存步骤,确认后调 `confirmChatTool` |
| `context_cleaned` | `:628` | 上下文刻度条变化提示 |
| `agent_start` / `agent_end` | `:681/:694` | 创建/收口 `AGENT` 步骤,按 `owner`/`invId` 归位 |
| `ui` | `:677` | `kb.references` 等 UI 产物,按 `messageId`+`name` 归位到引用区 |
| `done` / `error` / `cancelled` / `interrupted` | `:376/:383/:390/:396` | 收口 loading、渲染终态/失败原因 |

**交互要点**:
- **前端用 `owner` 判层级**:事件 `owner` 缺省 → 顶层;非空 → 嵌进 `agentCode == owner` 或 `invId == owner` 的 agent step。前端不自己维护嵌套关系,完全信任事件字段。
- **`args` 双时刻脱敏**:`tool_start` 与 `tool_end` 的 args 必须同为脱敏后的同一份,否则前端在工具结束瞬间看到入参跳变;`args` 为空时 `ToolStep` 整个 body 不渲染(长耗时工具看不到"在跑什么")。
- **UI 产物是封闭信封**:`type=ui` 仅接受 `UiArtifactNames` 已登记的 name(`kb.references` / `run.tokenUsage`),前端用 `isSupportedUiArtifact` 过滤;`kb.references` 按 `messageId` 合并成一行引用(乐观锁防并行丢更新)。
- **终态字段约定**:终态事件(`done`/`error`/`cancelled`/`interrupted`)一律带 `status`,前端据此判断"正常结束";`done` 的 `usage`/`context` 可选项让刻度条省一次 `GET context` 请求。
- **子 agent 事件归位靠 `invId`**:同一子 agent 一轮被调多次时,`owner=invId`(调用实例)而非 `agentCode`(同名卡片),前端据此把第 2 次调用的步骤挂到正确的卡片下。
