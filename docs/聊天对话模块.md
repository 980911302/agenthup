...
聊天模块是**前后端交互最复杂**的模块:一条通道做会话/消息的读写(REST),另一条通道做实时事件(WebSocket JSON-RPC),两条在 `sessionId` 上汇合。

**发起一轮对话(REST 入口)**:

```
Chat 页输入框
  → useChatRun.send()          [ruoyi-ui/src/views/ai/chat/composables/useChatRun.js]
  → createChatRun({sessionId, agentId, inputText, attachments})   [chat.js:30]
  → POST /ai/chat/run          [AiChatRunController]
  → 返回 runId(立即),执行在服务端线程池,事件随后经总线广播
```
...
| 阶段 | 前端 | RPC 方法 | 后端 |
| --- | --- | --- | --- |
| 建连 | `createChatWebSocketTicket()` → 带 `ticket` 打开 `/ws/ai/chat` | — | `ChatWebSocketTicketService` 签发一次性票据 |
| 订阅会话 | `useChatRun.watchSession(sessionId)` | `chat.session.subscribe` | `ChatSubscriptionRegistry` 登记,后续新 run 的 `runId` 推给前端 |
| 订阅运行 | `subscribe(runId, afterSeq)` | `chat.run.subscribe` | `ChatRunEventBroker.replay(runId, afterSeq)` 补齐断点 |
| 收事件 | `chat.event` | — | `ChatEventJson`(text/reasoning/toolStart/done...)逐条推 |
| 工具确认 | 弹确认框 → `confirmChatTool(runId, confirmId, approved)` | — | 确认结果写回挂起的工具调用 |
| 取消 | `useChatRun.abort()` | `chat.run.unsubscribe` + `POST /run/{runId}/cancel` | 服务端中断并广播终态 |

**会话管理(REST,纯读写)**:列表/时间线/上下文用量/清空记忆/回滚最后一轮/工具结果/追踪 全部走 §4.1 的 HTTP 端点;`timeline` 一次性返回消息 + runs 状态,`useTurnBuilder.applyRunStates` 把失败/取消轮收成终态。

**交互要点**:
- **一次聊天 = 1 个 REST 创建 + N 个 WS 事件**:前端不轮询;`runId` 是 REST 与 WS 的对账锚点,`afterSeq` 支持断线续传。
- **多端共享同一会话**:`chat.session.subscribe` 让所有打开该会话的标签页/桌面端/插件都收到广播,`chatRpc.js` 用引用计数管理连接生命周期。
- **连接状态可视化**:顶栏「连接指示灯」反映 WebSocket 状态(连接中/已连接/掉线重连),`chatRpc` 自动重连 + 指数退避。
- **前端入口在多客户端复用**:desktop(`desktop/src/chat/chatRpc.js` 等)与 extension 复用同一协议(见 `渠道工具与浏览器插件.md`),仅 API 封装路径不同。
