...
**前端页面**:`ruoyi-ui/src/views/index.vue`(运营总览首页,无独立 stat 菜单页);**API 封装**:`ruoyi-ui/src/api/ai/stat.js`。

| 前端板块 | API 函数(`stat.js`) | HTTP | 渲染内容 |
| --- | --- | --- | --- |
| KPI 卡(总 token/调用/会话/真实率) | `getStatOverview(days)` | GET `/ai/stat/overview` | `overview.totalTokens` / `promptTokens` / `completionTokens` / `callCount` / `sessionCount` / `realRate` |
| 模型 TopN | `getStatByModel(days, limit)` | GET `/ai/stat/by-model` | 条形图,按 `totalTokens` 降序 |
| 智能体 TopN | `getStatByAgent(days, limit)` | GET `/ai/stat/by-agent` | 条形图,`LEFT JOIN ai_agent` 取名 |
| 趋势折线 | `getStatTrend(days)` | GET `/ai/stat/trend` | 自绘 SVG 折线(`trendPoints` / `trendXTicks`) |
| 缓存命中卡 | `getStatCache(days)` | GET `/ai/stat/cache` | `cache.hitRate` + 命中/未命中 token |
| 任务成功率 | `getStatRuns(days)` | GET `/ai/stat/runs` | `successRate` + 状态分布环图 + 平均耗时 |
| 渠道健康 | `getStatChannelsHealth()` | GET `/ai/stat/channels-health` | 健康率 chip + 健康/启用/总计数,点击跳渠道管理页 |
| MCP 健康 | `getStatMcpHealth()` | GET `/ai/stat/mcp-health` | 健康率 chip,点击跳 MCP 管理页 |

**交互要点**:
- **挂载即并发拉取**:`index.vue` 8 个接口同时发起(默认 `days=7`,趋势 `days=30`),无懒加载、无轮询;数据是 `GROUP BY + sum/count` 实时算出来的,没有离线汇总表,所以每次刷新都是当下真实值。
- **数字格式化在前端**:`formatTokens`(token 的 K/M 缩写)、`formatDuration`(耗时)、`formatNum` 都在 `index.vue` 内做,后端只返回原始数值。
- **健康卡可下钻**:渠道/MCP 健康块带 `@click` 跳转到对应管理页(`/ai/model/channel`、`/ai/cap/mcpServer`),是总览页与管理面的入口桥。
- 无 WebSocket/SSE;所有 stat 为同步 REST,`AiStatController` 按当前登录人隔离 `user_id`(个人 `my-usage` 也同源)。
