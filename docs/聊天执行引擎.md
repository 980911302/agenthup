...
| `POST` | `/{runId}/tool-result` | `runService.completeChannelTool(runId, ...)` | 渠道工具结果回传(`{callId[≤100 必填], ok[必填], result, error, mediaFileId}`),与 tool-confirm 对称;callId 不在挂起表返回 `matched=false` |
