...
### 4.4 前后端交互(Run 生命周期视角)

**核心原则**:前端不持有执行生命周期,只**跟随状态机 + 订阅事件**。`useChatRun` 是唯一的前端语义封装,下面按 Run 状态机串起交互。
...
| Run 状态 | 前端动作 | 通道 | 说明 |
| --- | --- | --- | --- |
| QUEUED | `send()` 收到 runId,立即渲染 user 气泡 + "排队中" | REST `POST /ai/chat/run` | 409(已有 active run)→ `getActiveChatRun` 二次确认 → `resume` |
| RUNNING | `chat.run.subscribe(runId, afterSeq)` → 收 `text/reasoning/toolStart` 流式渲染 | WS `chat.run.subscribe` | `afterSeq` 断点续传;replay 后补发挂起的渠道工具请求 |
| 工具确认 | 弹确认框 → `confirmChatTool(runId, confirmId, approved)` | REST | 挂起中的工具等确认才继续 |
| FINALIZING | 渲染收尾(引用/摘要/UI 产物) | WS `chat.event` | 前端不区分 FINALIZING 与终态,按事件渲染 |
| SUCCEEDED | `done` 事件 → 停止 loading,时间线锚定 | WS `chat.event` | `handledRunIds` 去重防止双发重建 |
| FAILED / CANCELLED / INTERRUPTED | 渲染失败原因/取消态 | WS + `reconcileTimer` 兜底 | 对账见下 |

**对账(reconcile)是前端最重要的鲁棒机制**:
- `reconcileTimer` 每 5s 调 `GET /ai/chat/run/latest` 比对本地 run 状态,发现不一致(如 WS 丢包、页面刷新错过的终态)就纠正 UI。
- 历史轮对账靠 timeline 返回的 `runs` 映射(见 `聊天对话模块.md §3.3`):失败/取消的一轮没有 `ASSISTANT_FINAL` 消息,只能凭 `ai_chat_run.status` 判断,`useTurnBuilder.applyRunStates` 收成终态。
- **detach vs abort 语义**:切走页面/换会话 → `detach()`(仅 `chat.run.unsubscribe`,不打断服务端执行);用户主动停止 → `abort()`(`POST /{runId}/cancel`,服务端中断并广播终态)。
- **断线/刷新**:WS 断开后 `chatRpc` 自动重连,`recoverSession(sessionId)` → `getActiveChatRun` → `resume` 订阅,Redis Stream 按 `afterSeq` 回放补齐,无白闪。
- **多端同步**:`chat.session.subscribe` 让所有打开会话的客户端都能收到新 runId 并自行订阅(见 `聊天对话模块.md §4.4`)。
