...
> 跨会话长期记忆(`ruoyi-system/.../ai/memory/longterm/` 包 + `ChatTurnRunner` 读侧注入 + `ContextCompactor` 写侧搭车 + `sql/init/10-12`)见本文档 §7「跨会话长期记忆」。
...
本模块没有独立菜单页,前后端交互散落在 **Chat 页**与**长期记忆管理页**两块。

### 4.5.1 Chat 页:时间线还原 + 上下文刻度条

| 前端用途 | 前端 | HTTP | 后端 |
| --- | --- | --- | --- |
| 还原历史会话 | 切换会话 → `getSessionTimeline(sessionId)` | `GET /ai/chat/session/{sessionId}/timeline` | `AiChatSessionController`,读 `ai_chat_message`(columnsLite) |
| 展开工具正文 | 点工具卡片"查看全文" → `getToolResult(...)` | `GET .../message/{messageId}/tool-result` | 按 `tool_result_path` 读外置正文 |
| 上下文刻度条 | `ContextMeter.vue` 的 `usage` prop | 取自 `done` 事件 `context` 或 `GET /ai/chat/session/{sessionId}/context` | `ContextUsageAnalyzer.analyze` → `ChatTurnRunner.buildContextUsage` |
| 清空记忆 | 会话菜单 → `clearSession(sessionId)` | `DELETE /ai/chat/session/{sessionId}/memory` | 清 `ai_chat_message` + 解绑计量 + 清文件/预算 |
| 回滚一轮 | 会话菜单 → `rollbackLastTurn(sessionId, agentId)` | `DELETE /ai/chat/session/{sessionId}/last-turn` | 删最后一轮 USER 之后消息 |

**交互要点**:
- **时间线是"轻量 + 按需展开"**:`timeline` 用 `columnsLite` 只下发 `tool_result`/`tool_args` 预览(200/500 字符)+ `tool_result_length`,前端点「查看全文」才按 `messageId` 拉完整正文 —— 避免一次时间线拉回几百 KB 工具结果。
- **刻度条数据源两处**:流式时吃 `done` 事件附带的 `context`(省一次请求);历史/刷新后调 `getContextUsage(sessionId, agentId)` 补取。`ContextMeter` 用 `used/budget/threshold/trustworthy` 渲染,`trustworthy=false` 时显示 `≈` 估算口径(见 §3.3)。
- **前端按 `messageType` 渲染**:USER/ASSISTANT 进气泡、TOOL 进可折叠卡片、THINKING 进思考条、SUMMARY 不渲染(压缩摘要行不打扰用户),`specialEvents` 摘要单独挂(见 §6 场景 C)。
- **附件还原**:USER 图片附件由前端拿 `attachments` JSON 再调工作区接口取文件;模型侧重建另受 `ConversationVisionGate` 约束。

### 4.5.2 长期记忆管理页(管理端)

| 前端操作 | API 函数(`memory.js`) | HTTP | 后端端点 |
| --- | --- | --- | --- |
| 台账列表 | `listMemory(query)` | GET | `/ai/memory/list` |
| 删除记忆 | `delMemory(memoryIds)` | DELETE | `/ai/memory/{memoryIds}` |

管理页(`ruoyi-ui/src/views/ai/memory/index.vue`)列出跨会话长期记忆台账(用户/agent 维度),支持按条件筛选与删除;删除时服务端**同步清理 PostgreSQL 关联向量**,前端无额外操作。**普通用户无感知**:读侧注入由 `ChatTurnRunner` 每轮自动完成,前端看不到"记忆检索"过程,只可能通过回答质量感受到记忆生效。
...
> 本节按当前代码实况描述,早期 v1(纯工具级 + Neo4j 双写 + 独立 Maven 模块)已推翻,现为 v2 全自动方案。
