...
**前端入口**:`ruoyi-ui/src/views/ai/chat/components/WorkspacePanel.vue`(聊天页右侧工作区面板)+ `WorkspacePreviewDialog.vue`(文件预览弹窗)+ `composables/useWorkspace.js` + `workspaceChanges.js`(变更提示);**API 封装**:`ruoyi-ui/src/api/ai/workspace.js`。

| 前端操作 | API 函数(`workspace.js`) | HTTP | 后端端点 |
| --- | --- | --- | --- |
| 加载目录树 | `getWorkspaceTree(sessionId)` | GET | `/ai/chat/workspace/{sessionId}/tree` |
| 预览文件 | `getWorkspaceFile(sessionId, path)` | GET | `/ai/chat/workspace/{sessionId}/file` |
| 上传文件 | `uploadWorkspaceFile(sessionId, file, onProgress)` | POST | `/ai/chat/workspace/{sessionId}/upload` |
| 删除文件 | `deleteWorkspaceFile(sessionId, path)` | DELETE | `/ai/chat/workspace/{sessionId}` |
| 清空工作区 | `clearWorkspace(sessionId)` | DELETE | `/ai/chat/workspace/{sessionId}` |
| 下载/打包 | `workspaceFileDownloadUrl` / `workspaceZipDownloadUrl` | GET | `/download` / `/download-zip` |

**交互要点**:
- **面板挂在哪,身份就取哪**:`WorkspacePanel` 属于 Chat 页,`sessionId` 来自当前会话(项目会话传 `projectId` 归并到项目工作区)。面板在消息流发送完成后自动刷新目录树。
- **上传复用进度回调**:`uploadWorkspaceFile` 带 `onProgress`,界面显示上传百分比;只新增不覆盖,同名冲突时用户需先删或改名。
- **预览是有上限的文本通道**:`getWorkspaceFile` 只回文本(200KB 上限)与 binary 标记,大文件/二进制走 `download` 原样下载,预览弹窗对图片类直接渲染下载 URL。
- **下载统一走 blob**:`download-zip` 用 `proxy.$download.zip` 收 `blob`,从而能区分真文件与 JSON 错误(见 §4 注释);下载整个会话/单目录时 `path` 参数决定范围。
- **清空工作区受 active run 保护**:前端点「清空」→ `DELETE`,若该会话正在跑 Run 会收到 `ServiceException`,面板提示后不执行 —— 避免正在写文件的工具被连锅端。
- **变更提示与工具写入联动**:`workspaceChanges.js` 消费工具执行事件,发现 AI 写了新文件时提示"模型生成了文件",用户可一键打开预览。
